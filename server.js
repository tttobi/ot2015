...
if(process.env.NODE_ENV == "prod") {
  io.configure( function(){
    io.set('log level', 1);
    io.set( 'origins', '*ot2015.herokuapp.com*:*' );
  });

  mongoose.connect(process.env.MONGOLAB_URI);
} else {
  mongoose.connect('mongodb://localhost:27017/ot2015');
}
...
require("./fixtures/ot2015.js").addFixtures(Tutor, Course, Group);
...
server.listen(process.env.PORT || 8000, process.env.IP || '0.0.0.0', function () {
  console.log('Express server listening on %s:%d', process.env.IP || '0.0.0.0', process.env.PORT || 8000);
});